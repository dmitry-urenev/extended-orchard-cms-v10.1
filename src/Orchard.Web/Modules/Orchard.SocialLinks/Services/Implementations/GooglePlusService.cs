using Orchard.DisplayManagement;
using Orchard.ContentManagement;
using Orchard.DisplayManagement.Shapes;
using Orchard.SocialLinks.Models;
using System;
using System.Collections.Generic;
using System.Linq;
using System.Web;
using Orchard.Core.Title.Models;
using Orchard.ContentManagement.Aspects;
using System.Collections.Specialized;

namespace Orchard.SocialLinks.Services.Implementations
{
    public class GoogleService : BaseSocialShareService, ISocialShareService
    {
        public GoogleService(IOrchardServices orchardServices)
            : base(orchardServices)
        {
        }

        public string Name
        {
            get { return "googlePlus"; }
        }

        public override string BaseServiceUrl
        {
            get { return "https://plus.google.com/share"; }
        }

        public Shape Apply(SocialContext context)
        {
            var map = new Dictionary<string, string> { 
                { "url", "url" }
            };
            var parameters = base.GetParameters(context, map);
            var shareUrl = base.ToUrl(parameters);

            var shape = Services.New.Parts_Share_Google(ServiceName: Name, Url: shareUrl, Parameters: parameters);
            return shape;
        }
    }
}