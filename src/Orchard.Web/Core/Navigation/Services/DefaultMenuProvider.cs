            ...
            foreach (var menuPart in menuParts) {
                if (menuPart != null) {
                    ...
                    if (part.Is<MenuItemPart>())
                        builder.Add(new LocalizedString(HttpUtility.HtmlEncode(part.MenuText)), part.MenuPosition, item => item.Url(part.As<MenuItemPart>().Url).TargetBlank(part.As<MenuItemPart>().TargetBlank).Content(part).Culture(culture).Permission(Contents.Permissions.ViewContent));
                    else
